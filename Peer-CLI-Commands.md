...
**Note:** Recommended approach to package chaincode is via `package-chaincode-job` under `Chaincode-Deployment` directory.<br> 
**SKIP BELOW PROCESS OF PACKAGING CHAINCODE, IF ALREADY DONE VIA K8s JOB.**
...
# For Node Chaincode
peer lifecycle chaincode approveformyorg --channelID samplechannel --name smartcontract-node --version 1.0 --package-id smartcontract-node:<Copied-PkgID> --sequence 1 -o orderer:7050 --tls --cafile $ORDERER_CA

# For Go Chaincode 
peer lifecycle chaincode approveformyorg --channelID samplechannel --name smartcontract-go --version 1.0 --package-id smartcontract-go:<Copied-PkgID> --sequence 1 -o orderer:7050 --tls --cafile $ORDERER_CA
```
